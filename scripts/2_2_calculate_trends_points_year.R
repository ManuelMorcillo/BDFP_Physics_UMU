...
for (i in 1:length(combinations)){
  ...
  if (combinations[i]=="C0"){
    auxC0 = sens.slope(FWIC0_filtered$weighted_mean_anomaly)
    slopeC0 <- auxC0$estimates
    interceptC0 <- mean(FWIC0_filtered$weighted_mean_anomaly)-slopeC0*mean(FWIC0_filtered$time)
...
##########################################################################
## Trend analysis and visualization for C0 to C4 (standardized), plotting
## the annual mean FWI trend over years
##########################################################################

FWIC0_means <- rename(FWIC0_filtered,weighted_mean=weighted_mean_anomaly)
...
FWI_means <- bind_rows("C0"=FWIC0_means,
                       "C1"=FWIC1_means,
                       "C2"=FWIC2_means,
                       "C3"=FWIC3_means,
                       "C4"=FWIC4_means,.id="tibble")
...
FWI_mean_line <- ggplot(FWI_means,aes(x = time
  ...
  geom_point(shape=1)+
  geom_line()+
  scale_color_manual(values = c("C0" = "red2",
                                "C1" = "purple2",
                                "C2" = "cyan3",
                                "C3" = "chartreuse2",
                                "C4" = "orange")) +
  theme_light()+
  geom_abline(intercept = interceptC0,slope = slopeC0,color="red2")+
  ...
  annotate("text",x = 1991, y = 1.85 ,label = paste0("Trend C0 : " ,format(round(slopeC0*10,sig),nsmall = dec)," (C.I.: ",format(round(auxC0$conf.int[1]*10,sig),nsmall = dec),"/",format(round(auxC0$conf.int[2]*10,sig),nsmall = dec),") [z-score/decade]"))+
  annotate("text",x = 1991, y = 1.75 ,label = paste0("Trend C1 : " ,format(round(slopeC1*10,sig),nsmall = dec)," (C.I.: ",format(round(auxC1$conf.int[1]*10,sig),nsmall = dec),"/",format(round(auxC1$conf.int[2]*10,sig),nsmall = dec),") [z-score/decade]"))+
  annotate("text",x = 1991, y = 1.65 ,label = paste0("Trend C2 : " ,format(round(slopeC2*10,sig),nsmall = dec)," (C.I.: ",format(round(auxC2$conf.int[1]*10,sig),nsmall = dec),"/",format(round(auxC2$conf.int[2]*10,sig),nsmall = dec),") [z-score/decade]"))+
  annotate("text",x = 1991, y = 1.55 ,label = paste0("Trend C3 : " ,format(round(slopeC3*10,sig),nsmall = dec)," (C.I.: ",format(round(auxC3$conf.int[1]*10,sig),nsmall = dec),"/",format(round(auxC3$conf.int[2]*10,sig),nsmall = dec),") [z-score/decade]"))+
  annotate("text",x = 1991, y = 1.45 ,label = paste0("Trend C4 : " ,format(round(slopeC4*10,sig),nsmall = dec)," (C.I.: ",format(round(auxC4$conf.int[1]*10,sig),nsmall = dec),"/",format(round(auxC4$conf.int[2]*10,sig),nsmall = dec),") [z-score/decade]"))+ 
  guides(color = guide_legend(title.position = "top",title.hjust = 0.5,label.position = "left"))+
  labs(x = "Time [years]", y = "FWI [z-score]",color ="",)

ggsave(paste0(dir_out, "FWI_Trend_Annual_Mean_Points_Line_",
              combinations[1],
              "+",combinations[2],
              "+",combinations[3],
              "+",combinations[4],
