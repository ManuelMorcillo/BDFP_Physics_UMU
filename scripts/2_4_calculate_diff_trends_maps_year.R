...
for (i in 1:length(combinations)){
  ...
  if (combinations[i]=="C0"){
    ...
    FWIC0 <- FWI # Lets make a copy for the future

  } else if (combinations[i]=="C1"|combinations[i]=="C2"|combinations[i]=="C3"|combinations[i]=="C4") {
    ...
    FWI$FWI <- FWI_std$standardized_FWI
    intersection <- inner_join(FWI,FWIC0, by = c("time","latitude", "longitude"),relationship = "many-to-many")
    FWI_diff <- tibble(time=intersection$time,
                       latitude=intersection$latitude,
                       longitude=intersection$longitude,
                       FWI=intersection$FWI.x-intersection$FWI.y)
    ...
    plot_fwi_trend <- ggplot() +
      ...
      scale_fill_gradientn(name="Slope\n[z-score/decade]",colors = rev(brewer.pal(11, "BrBG")),limits=c(-0.4,0.4),breaks=seq(-0.4,0.4,0.1)) + # Color scale
      ...
      labs(
        title = paste0("Trend of the annual mean\nFWI for ",combinations[i],"-C0 (1980-2020)"),
        caption = "Database: ERA5",
        x = "Longitude [º]",
        y = "Latitude [º]"
      ) +
      theme_light() +
      theme(plot.margin = unit(c(0,0,0,0), "cm"),plot.title = element_text(size=14,face = "bold"),
            legend.text = element_text(size = 11.2),plot.caption = element_text(size = 13))+
    ...
    ggsave(paste0(dir_out, "FWI_Trend_Annual_Mean_Diff_Map_",combinations[i],"-C0.pdf"), plot_fwi_trend, width = 8, height = 6)
...
print(layout)
pdf(paste0(dir_out,"FWI_Trend_Annual_Mean_Diff_Maps.pdf"),width = 10, height = 8)
multiplot(plots$plot_C1, 
          plots$plot_C2,
          plots$plot_C3,
          plots$plot_C4,
          layout = layout )
